...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1 id="menu-title">Tindakan Dokter</h1>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li><a href="#">Data Master</a></li>
            <li class="active">Tindakan Dokter</li>
        </ol>
    </section>
    <!-- Main content -->
                ...
                <div class="box box-primary">
                    <div class="box-header">
                        <div class="form-inline">
                            <?php if($this->session->userdata('lvlPengguna') < 3 && $dokter){?>
                            <a href="<?php echo base_url('tindakandokter/tambah?dr='.$dr)?>" class="btn btn-sm btn-primary" ><span class="glyphicon glyphicon-plus"></span> Tambah</a>
                            <?php }?>
                            <div class="form-group input-group-sm">
                                <select class="form-control" id="filter-dokter" onchange="filter()">
                                    <?php 
                                        foreach ($dokter as $row) {
                                            echo "<option value='".$row->idDokter."'";
                                            if($row->idDokter == $dr)
                                                echo "selected";
                                            echo ">".$row->namaDokter."</option>";
                                        }
                                    ?>
                                </select>
                            </div>
                                ...
                                <?php 
                                    if($data){
                                        foreach ($data as $row) {
                                            ...
                                            echo "<td align='right'>";
                                            echo "<a href='".base_url('tindakandokter/edit?id='.$row->idTindakan)."' class='btn btn-default btn-xs'><i class='fa fa-pencil'></i></a>";
...
<script type="text/javascript">    
    function filter(){
        window.location = base_url + 'tindakandokter/index.html?q=' + $("#filter-cari").val() + '&dr=' + $("#filter-dokter").val();
    }
</script>