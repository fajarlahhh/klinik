...
class Penjualan extends CI_Controller {
    ...
    {
        $this->sessioncheck->validasi('penjualan', $this->redirect);
        $q = $this->input->get('q');
        ...
        $config = array(
            'base_url' => site_url('penjualan/data?bln='.$bln.'&thn='.$thn.'&q='.$q),
            'first_url' => site_url('penjualan/data?bln='.$bln.'&thn='.$thn.'&q='.$q),    
            'per_page' => 10,
        ...
        $this->load->view('apotek/penjualan/index', $content);
    ...
    public function input($action = null)
    {
        ...
        if($action){
            if($this->session->userdata('lvlPengguna') < 3){
                ...
                if ($this->db->trans_status() === FALSE){
                    ...
                    $this->session->set_flashdata('message', $dlg);
                    redirect('penjualan/cetak?no='.$nomor);
                }
            }else{
        ...
        $barang = $this->mdatabarang->get_tidak_khusus();
        ...
        $this->load->view('include/header');
        $this->load->view('include/sidebar');
        $this->load->view('apotek/penjualan/form', $content);
        $this->load->view('include/footer');
    }
